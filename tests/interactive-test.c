...
TEST(focus_tests)
{
	...
	assert(c1->input->pointer->x == 0);
	assert(c1->input->pointer->y == 0);
	...
	simulate_move(c1, 100 + GRAB_SHIFT_X, 100 + GRAB_SHIFT_Y, 50, 50);
	assert(c1->input->pointer->focus == NULL);
	assert(c1->input->keyboard->focus == c1->surface->wl_surface);
...
TEST(maximize_client_by_double_click)
{
	...
	simulate_move(client, 100, 100, 250, 100 + GRAB_SHIFT_Y);
	/* make sure we're in frame */
	assert(client->input->pointer->x == 0);
	assert(client->input->pointer->y == 0);
